...
- What the app is (deployable process), what it does, public surface (routes, job types, commands), dependencies (which `@clickjournal` packages it uses), consumers, local dev, production story.

### Package READMEs (`packages/*/README.md`)
- What the package is (shared library), public API (exports table), dependencies (other `@clickjournal` packages + external), consumers, usage notes (env vars, configuration), when to add something new.
