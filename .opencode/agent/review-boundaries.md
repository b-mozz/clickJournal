...
- `apps/worker` cannot import from `@clickjournal/domain` or `@clickjournal/auth` (current boundary).
- `packages/domain` cannot import from `apps/*` (it's web-only).
- Prisma client imported from `@clickjournal/db` only — never from `@prisma/client` directly.
